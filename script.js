var winWidth = $(window).width();
var ratio = winWidth / 1920;
var fontSize = {
  small: 12,
  medium: 14
};
var played = [0, 0, 0];
var vara = [];
var bodyFontSize = Math.max(16 * ratio, 10);
var posX = Math.max(80 * ratio, 30);
$("body").css("font-size", bodyFontSize + "px");
fontSize.small = Math.max(fontSize.small * ratio, 7);
fontSize.medium = Math.max(fontSize.medium * ratio, 10);
vara[0] = new Vara(
  "#vara-container",
  "https://rawcdn.githack.com/akzhy/Vara/ed6ab92fdf196596266ae76867c415fa659eb348/fonts/Satisfy/SatisfySL.json",
  [
    {
      text: "13 May 2023",
      textAlign: "right",
      y: 20,
      x: -30,
      delay: 500,
      duration: 1500,
      fontSize: fontSize.small
    },
    {
      text: "Congratulations on reaching this important achievement in your academic journey!",
      y: 40,
      x: posX,
      duration: 20000,
      fontSize: fontSize.small
    },
    {
      text: " Your hard work has paid off and you've accomplished a great milestone. As you move forward, keep in mind the knowledge and skills you've gained. ",
      id: "sphinx",
      x: posX,
      delay: 3000,
      duration: 20000,
      fontSize: fontSize.small
    },
    {
      text: " ",
      id: "end",
      color: "#3f51b5",
      delay: 1000,
      x: posX,
      duration: 4500
    }
  ],
  {
    strokeWidth: 2,
    fontSize: fontSize.medium,
    autoAnimation: false
  }
);
vara[1] = new Vara(
  "#vara-container2",
  "https://rawcdn.githack.com/akzhy/Vara/ed6ab92fdf196596266ae76867c415fa659eb348/fonts/Satisfy/SatisfySL.json",
  [
    {
      text: "13 May 2023",
      textAlign: "right",
      delay: 500,
      y: 20,
      x: -30,
      duration: 1500,
      fontSize: fontSize.small
    },
    {
      text: " Always keep striving for excellence, and never lose sight of your dreams.",
      y: 40,
      x: posX,
      duration: 20000,
      fontSize: fontSize.small
    },
    {
      text: "Never stop pursuing your passions",
      y: 40,
      x: posX,
      duration: 15000,
      fontSize: fontSize.small
    },
    {
      text: " You should be incredibly proud of all that you have accomplished!",
      y: 30,
      x: posX,
      duration: 20000,
      fontSize: fontSize.small
    }
  ],
  {
    strokeWidth: 2,
    fontSize: fontSize.medium,
    autoAnimation: false
  }
);
vara[2] = new Vara(
  "#vara-container3",
  "https://rawcdn.githack.com/akzhy/Vara/ed6ab92fdf196596266ae76867c415fa659eb348/fonts/Satisfy/SatisfySL.json",
  [
    {
      text: "13 May 2023",
      textAlign: "right",
      delay: 500,
      y: 20,
      x: -30,
      duration: 1500,
      fontSize: fontSize.small
    },
    {
      text: "Your happiness and success are defined by you, so trust yourself and follow your instincts to achieve your goals and live a meaningful life.",
      y: -10,
      x: posX,
      duration: 30000,
      fontSize: fontSize.small
    },
    {
      text: "May your future filled with success, happiness, and endless opportunities. Best wishes for a bright and fulfilling future ahead!",
      y: -70,
      x: posX,
      duration: 30000,
      fontSize: fontSize.small
    },
    {
      text: "Proud of You icil             ~Engel",
      y: -68,
      color: "#B87333",
      x: posX,
      duration: 30000,
      fontSize: fontSize.small
    }
  ],
  {
    strokeWidth: 2,
    fontSize: fontSize.medium,
    autoAnimation: false
  }
);
vara[2].ready(function() {
  $(".front:not(.last)").click(function() {
    var ix = $(this)
      .parent(".paper")
      .index();
    $(".book").addClass("open");
    $(this)
      .parent(".paper")
      .addClass("open");
    if (!played[ix]) {
      vara[ix].playAll();
      vara[ix].animationEnd(function(i, o) {
        played[ix] = 1;
        if (i == "link") {
          var group = o.container;
          var rect = vara[2].createNode("rect", {
            x: 0,
            y: 0,
            width: o.container.getBoundingClientRect().width,
            height: o.container.getBoundingClientRect().height,
            fill: "transparent"
          });
          group.appendChild(rect);
          $(rect).css("cursor", "pointer");
          $(rect).click(function() {
            console.log(true);
            document.querySelector("#link").click();
          });
        }
      });
    }
  });
  $(".back").click(function() {
    if (
      $(this)
        .parent(".paper")
        .index() == 0
    )
      $(".book").removeClass("open");
    $(this)
      .parent(".paper")
      .removeClass("open");
  });
});
